...
def _create_text_labels(classes, scores, class_names, is_crowd=None):
    ...
    #class_names = ['Infiltrate', 'Atelectasis', 'Pneumonia', 'Cardiomegaly', 'Effusion', 'Pneumothorax', 'Mass', 'Nodule']
    labels = None
    if classes is not None:
        if class_names is not None and len(class_names) > 0:
            labels = [class_names[i+1] for i in classes]
        else:
...
class Visualizer:
    ...
    def draw_instance_predictions(self, predictions, jittering: bool = False):
        """
        ...
        if self._instance_mode == ColorMode.SEGMENTATION and self.metadata.get("thing_colors"):
            ...
            #CLASS_COLORS = {0: (0.0, 0.0, 1.0), 2: (0.0, 1.0, 0.0)}
            CLASS_COLORS = {
                0: (1.0, 0.0, 0.0),  # Red
                1: (0.0, 1.0, 0.0),  # Green
                2: (0.0, 0.0, 1.0),  # Blue
            ...
            colors = [CLASS_COLORS.get(c, (1.0, 1.0, 1.0)) for c in classes]
