    ...
    if args.input:
        ...
        with open(args.csv_path, "w", newline="") as csv_file:
            ...
            if len(args.input) == 1:
                ...
                input = natsorted(args.input)
            for path in tqdm.tqdm(input, disable=not args.output):
                ...
                if boxes:
                    for box, score, cls in zip(boxes, scores, classes):
                        ...
                        writer.writerow([os.path.basename(path), box_str, score.item(), cls.item()])
                else:
                    writer.writerow([os.path.basename(path), None, None, None])
